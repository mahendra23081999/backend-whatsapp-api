...
export function createInstallationMiddleware() {
  return async (req, res, next) => {
    console.log('[Installation Middleware] req.originalUrl:', req.originalUrl);
    if (
      req.originalUrl.startsWith('/install') ||
      req.originalUrl.startsWith('/uploads') ||
      req.originalUrl.startsWith('/api') ||
      req.originalUrl === '/favicon.ico' ||
      req.originalUrl === '/health'
    ) {
      console.log('[Installation Middleware] Skipping for:', req.originalUrl);
      return next();
    }
